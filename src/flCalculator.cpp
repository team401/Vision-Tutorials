 ...
 * Date: 3.15.17
 *
 * Calculates the focal length of your camera
 */
...
int main(int argc, char **argv) {
    ...
    double distance = atoi(argv[0]);    // Diagonal inches from the camera to the center of the retro reflective tape
    ...
    for(;;) {
        ...
        if(goodContours.size() >= 2) {
            ...
            // Calculates focal length from distance
            focalLength = distance * (bottomCenter.y - topCenter.y)/10;
        }
